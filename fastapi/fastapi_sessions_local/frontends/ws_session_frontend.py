...
class SessionFrontend(ABC, Generic[ID]):
    ...
    def attach_id_state(self, websocket_: WebSocket, session_id: Union[ID, FrontendError]):
        """Attach the extracted session id to the request."""
        try:
            websocket_.state["session_ids"][self.identifier] = session_id 
            # request.state.session_ids[self.identifier] = session_id
        except Exception:
            websocket_.state["session_ods"] = {}
            websocket_.state["session_id"][self.identifier] = session_id

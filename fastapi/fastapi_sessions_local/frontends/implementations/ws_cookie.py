...
class SessionCookie(SecurityBase, SessionFrontend[UUID]):
    def __init__(
        ...
        *,
        ...
        logging.debug(f"Вызван констуктор класса {self.__class__.__name__}")

    @property
    def identifier(self) -> str:
        return self._identifier

    def __call__(self, websocket_: WebSocket) -> Union[UUID, FrontendError]:
        # Get the signed session id from the session cookie
        logger.debug(f"Вызван метод call у класса {self.__class__.__name__}")
        signed_session_id = websocket_.cookies.get(self.model.name)
        print(f"{signed_session_id}")
        ...
        # super().attach_id_state(websocket_, session_id)
        return session_id
    ...
    def attach_to_response(self, response: Response, session_id: UUID) -> None:
        print("session" + self.signer.dumps(session_id.hex))
        response.set_cookie(
            key=self.model.name,
